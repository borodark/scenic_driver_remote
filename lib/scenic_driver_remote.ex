defmodule ScenicDriverRemote do
  ...
  Set `size: {w, h}` to your **scene's design size** — the reference resolution
  your scene was authored for (e.g. `@base_width 1668, @base_height 2388`).

  The scene renders at 1:1 in its design space. When a remote client connects
  and reports its actual screen size via RESHAPE, the driver computes a
  **GLOBAL_TX** transform that scales and centers the design canvas onto the
  device screen (uniform scale, letterbox fit). This works for any device —
  no per-device configs needed.

      # Scene design size (matches @base_width / @base_height)
      size: {1668, 2388}
  ...
  1. The driver computes a **GLOBAL_TX** affine transform:

         scale = min(device_w / vp_w, device_h / vp_h)
         tx    = (device_w - vp_w * scale) / 2
         ty    = (device_h - vp_h * scale) / 2

  2. This is sent to the client as `global_tx(scale, 0, 0, scale, tx, ty)`.

  The scene does **not** need to handle reshape — GLOBAL_TX handles adaptation
  to any screen size automatically.
